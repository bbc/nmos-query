...
sender_format = {
    ...
    'device_id': re.compile('^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'),
    'manifest_href': 'uri',
    'interface_bindings': [re.compile('eth\d')],
    'subscription': {
...
receiver_format = {
    'video': {
        ...
        'version': re.compile('^[0-9]+:[0-9]+$'),
        'label': 'String Name',
        'description': 'A longer string describing thing',
        'tags': {},
        'device_id': re.compile('[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'),
        'transport': re.compile('urn:x-nmos:transport:(rtp|rtp\.ucast|rtp\.mcast|dash)'),
        'interface_bindings': [re.compile('eth\d')],
        'subscription': {
            'sender_id': re.compile('^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'),
            'active': True
        },
        'format': 'urn:x-nmos:format:video',
        'caps': [re.compile('video\/(raw|H264|vc2)')]
    },
    'audio': {
        'id': re.compile('[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'),
        'version': re.compile('^[0-9]+:[0-9]+$'),
        ...
        'format': 'urn:x-nmos:format:audio',
        'caps': [re.compile('audio\/(L24|L20|L16|L8)')]
    },
    'data': {
        'id': re.compile('[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'),
        'version': re.compile('^[0-9]+:[0-9]+$'),
...
def generate_receiver():
    receiver_types = list(receiver_format.keys())
    receiver_type = receiver_types[randint(0, len(receiver_types) - 1)]
    return generate_object(receiver_format[receiver_type])
